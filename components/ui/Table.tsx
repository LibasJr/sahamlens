...
export function TableHead({ className, children, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) { return <th className={cn('px-3 py-3 text-left text-[12px] font-bold uppercase tracking-[0.1em] text-tv-muted sm:px-4 sm:text-[10px] sm:tracking-[0.12em]', className)} {...props}>{children}</th>; }
interface TableCellProps extends React.TdHTMLAttributes<HTMLTableCellElement> { numeric?: boolean; }
export function TableCell({ numeric = false, className, children, ...props }: TableCellProps) { return <td className={cn('px-3 py-3 text-sm leading-relaxed text-tv-text sm:px-4', numeric && 'text-right font-number tabular-nums', className)} {...props}>{children}</td>; }
