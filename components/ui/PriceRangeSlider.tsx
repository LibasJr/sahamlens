...
export function PriceRangeSlider({
  ...
  return (
    <div className={`p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] select-none ${className}`}>
      {/* Header Info */}
      <div className="flex items-center justify-between text-xs mb-2">
        <span className="font-semibold text-tv-muted">{label}</span>
        <span className="font-number font-bold text-white text-[11px]">
      ...
      <div className="relative h-2 w-full rounded-full bg-white/[0.08] overflow-visible my-3">
      ...
      <div className="flex items-center justify-between text-[11px] font-number text-tv-muted pt-0.5">
        <div>
          ...
          <strong className="text-white">Rp {lowPrice.toLocaleString('id-ID')}</strong>
        </div>
        ...
        <div className="text-right">
          ...
          <strong className="text-white">Rp {highPrice.toLocaleString('id-ID')}</strong>
        </div>
      </div>
    </div>
  );
