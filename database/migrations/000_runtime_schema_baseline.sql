-- SahamLens migration baseline.
-- Captured from previously idempotent runtime DDL. DATA mutations/retention cleanup were
-- intentionally excluded: this baseline owns schema only. After it is applied, application
-- requests no longer create/alter these schemas. Applied migration files are immutable.

-- Legacy user/auth schema, frozen from runtime DDL on 2026-08-16.
...
-- Shared production schema, frozen from runtime DDL on 2026-08-16.
...
-- Intraday research schema, frozen from runtime DDL on 2026-08-16.
...
-- Admin credential table is migration-owned from this point onward.
