...
export default async function BankFundamentalsAdminPage() {
  ...
  return <div className="min-h-screen bg-tv-bg p-4 text-tv-text sm:p-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="mb-7 flex items-start gap-3"><div className="rounded-xl bg-tv-blue/10 p-2.5 text-tv-blue"><Building2 className="h-6 w-6"/></div><div><h1 className="font-heading text-2xl font-bold sm:text-3xl">Bukti Fundamental Bank</h1><p className="mt-1 max-w-3xl text-sm text-tv-muted">NIM, NPL, CASA, CAR, LDR, Cost of Credit, CIR, coverage, dan PPOP disimpan per metrik dengan sumber dan tanggal publikasi. Belum menjadi input LensScore.</p></div></div>
    ...
    <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Card as="div" padding="none" radius="xl" elevation="none" overflow="visible" highlight={false} className="border-tv-border p-4"><p className="text-xs text-tv-muted">Evidence rows</p><p className="mt-1 font-number text-2xl font-bold">{Number(totals.evidence_rows??0)}</p></Card>
      <Card as="div" padding="none" radius="xl" elevation="none" overflow="visible" highlight={false} className="border-tv-border p-4"><p className="text-xs text-tv-muted">Ticker</p><p className="mt-1 font-number text-2xl font-bold">{Number(totals.tickers??0)}</p></Card>
      <Card as="div" padding="none" radius="xl" elevation="none" overflow="visible" highlight={false} className="border-tv-border p-4"><p className="text-xs text-tv-muted">Period</p><p className="mt-1 font-number text-2xl font-bold">{Number(totals.periods??0)}</p></Card>
      <Card as="div" padding="none" radius="xl" elevation="none" overflow="visible" highlight={false} className="border-tv-border p-4"><p className="text-xs text-tv-muted">Observed terbaru</p><p className="mt-1 font-number text-base font-bold">{totals.latest_observed ? String(totals.latest_observed).slice(0,10) : '—'}</p></Card>
    </div>
    ...
    <Card as="div" padding="none" radius="xl" elevation="none" overflow="visible" highlight={false} className="mb-8 border-tv-border p-4 text-sm">
