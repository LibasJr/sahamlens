      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-10">
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            <Card><p className="text-xs text-tv-muted">Risk-free</p><p className="mt-1 font-number text-xl font-bold">{pct(macro.production.riskFreeRatePct)} → {pct(macro.candidate.riskFreeRatePct)}</p></Card>
            <Card><p className="text-xs text-tv-muted">ERP</p><p className="mt-1 font-number text-xl font-bold">{pct(macro.production.equityRiskPremiumPct)} → {pct(macro.candidate.equityRiskPremiumPct)}</p></Card>
            <Card><p className="text-xs text-tv-muted">Perpetual growth cap</p><p className="mt-1 font-number text-xl font-bold">{pct(macro.production.maxPerpetualGrowthPct)} → {pct(macro.candidate.maxPerpetualGrowthPct)}</p></Card>
          </div>
          ...
          <Card className="mt-3 text-sm text-tv-muted">
        ...
        <section>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4">
            <Card><p className="text-xs text-tv-muted">Ticker</p><p className="mt-1 font-number text-2xl font-bold">{bank.tickerCount}</p></Card>
            <Card><p className="text-xs text-tv-muted">Evidence rows</p><p className="mt-1 font-number text-2xl font-bold">{bank.evidenceRows}</p></Card>
            <Card><p className="text-xs text-tv-muted">Research-analyzable</p><p className="mt-1 font-number text-2xl font-bold">{bank.tickers.filter((row)=>row.researchAnalyzable).length}</p></Card>
            <Card><p className="text-xs text-tv-muted">Scoring enabled</p><p className="mt-1 font-number text-2xl font-bold">0</p></Card>
          </div>
          <Card padding="none" className="overflow-x-auto">
