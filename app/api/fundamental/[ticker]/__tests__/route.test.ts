...
describe('GET /api/fundamental/[ticker]', () => {
  ...
  it('cache hit tidak memanggil fetchCurrentFundamentalSource sama sekali', async () => {
    ...
    expect(res.status).toBe(200);
    // `meta.requestId` ADITIF dari runController (shared/http/next-response.adapter.ts):
    // setiap respons membawa id yang sama dengan baris lognya. Diasersikan terpisah, bukan
    // dilonggarkan jadi toMatchObject - kalau amplop standar itu hilang, tes ini gagal.
    const { meta, ...payload } = json;
    expect(payload).toEqual(cached);
