...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      relativeStrength: category(summary.topRelativeStrength || [], (s: any) => ({ symbol: s.symbol, price: s.price, changePct: s.changePct, metric: `RS ${s.relativeStrength5D >= 0 ? '+' : ''}${s.relativeStrength5D.toFixed(1)}% vs IHSG 5D` })),
