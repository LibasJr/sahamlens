...
const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  { pattern: /\b(ownership flow|arus kepemilikan|foreign ownership|kepemilikan)\b/, name: 'Arus Kepemilikan', function: 'melacak perubahan komposisi kepemilikan asing dan lokal antar-snapshot', usage: 'pilih emiten lalu baca perubahan, periode snapshot, dan status sumber', result: 'perubahan struktural komposisi kepemilikan', limitation: 'bukan broker flow dan saat ini tidak masuk LensScore' },
  ...
  // Transparansi DIPISAH dari Tentang & Pattern (23 Agustus 2026): sejak halamannya pindah
  // ke Admin, menyebutnya dalam satu tarikan napas dengan menu publik membuat LensAI
  // menyuruh pengguna biasa membuka menu yang tidak ada di sidebar mereka.
  { pattern: /\b(transparansi|transparency|bukti validasi)\b/, name: 'Bukti Validasi LensRadar', function: 'meninjau bukti forward per kelompok LensScore, uji signifikansi, dan rekonsiliasi harga lintas sumber', usage: 'panel internal - buka Admin → Bukti Validasi LensRadar; menu ini tidak tersedia untuk pengguna umum', result: 'bukti validasi model dan status point-in-time', limitation: 'angkanya tidak dipublikasikan, dan bukti historis tidak menjamin performa ke depan' },
...
function allFeaturesAnswer(): string {
  const adminStart = PRODUCT_FEATURES.findIndex((feature) => feature.name === 'Uji Akurasi LensRadar');
...
export function getDeterministicProductHelpResponse(prompt: string): string {
  ...
  const adminStart = PRODUCT_FEATURES.findIndex((item) => item.name === 'Uji Akurasi LensRadar');
