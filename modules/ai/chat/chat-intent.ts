...
const PRODUCT_TERMS = /\b(lensscore|lensradar|lenstechnical|lensfundamental|lensmarket|lensconsensus|lensai|sahamlens|screener|backtest|scoring|skor fundamental|skor teknikal|dcf|intrinsic value|nilai intrinsik|nilai intrinsic|nilai intrisik|nilai intric|multi-?agent|council|blue.?chip|small.?cap|cap tier|compare|portofolio|portfolio|watchlist|daily picks|peluang hari ini|provenance|sumber data|coverage|confidence|transparansi|transparency|glosarium|glossary)\b/;
...
const PRODUCT_FEATURE_DEFINITION_TERMS = /\b(beranda|home|lensmarket|market pulse|lensradar|daily picks|peluang hari ini|lenstechnical|lensscanner|compare|backtest|live filter check|lensfundamental|valuation|valuasi|dcf|nilai intrinsik|nilai intrinsic|moat|earnings|dividen|dividend|lenswatch|watchlist|akun demo|paper trading|risk matrix|risk calculator|news(?:\s*&\s*sentiment)?|berita|sentimen|corporate calendar|kalender|calendar|macro|makro|transparansi|transparency|metodologi|provenance|sumber data|coverage|confidence|tentang|about|pattern|pola|laporan keuangan|corporate action|broker flow|broker summary|foreign flow|domestic driven|arus dana|risk profile|manajemen risiko)\b/;
 ...
 */
const PRODUCT_RESEARCH_ADMIN_FEATURE_TERMS = /\b(kesehatan operasional|operational health|simulasi keputusan ai|decision lab|paper order|decision advisory|ownership flow validation lab|uji arus kepemilikan|bank fundamentals? evidence|bank fundamental evidence|bukti fundamental bank|financial integrity(?:\s*&\s*adoption gate)?|pemeriksaan data keuangan|adoption gate|fundamental backfill|impor histori fundamental|tp\s*\/?\s*cl validation lab|tpcl validation lab|uji target|intraday validation lab|uji intraday|lensradar calibration lab|uji akurasi lensradar|macro pit|macro assumptions|bukti data makro|data integrity|market data reconciliation|pemeriksaan harga penutupan|basis harga|price basis|bank fundamental collector|ownership flow status ingestion|infographic studio|studio infografis|infografis|kartu riset|export card)\b/;
