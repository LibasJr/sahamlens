 ...
 */
const PRODUCT_RESEARCH_ADMIN_FEATURE_TERMS = /\b(kesehatan operasional|operational health|ownership flow validation lab|uji arus kepemilikan|bank fundamentals? evidence|bank fundamental evidence|bukti fundamental bank|financial integrity(?:\s*&\s*adoption gate)?|pemeriksaan data keuangan|adoption gate|fundamental backfill|impor histori fundamental|tp\s*\/?\s*cl validation lab|tpcl validation lab|uji target|intraday validation lab|uji intraday|lensradar calibration lab|uji akurasi lensradar|macro pit|macro assumptions|bukti data makro|data integrity|market data reconciliation|pemeriksaan harga penutupan|bank fundamental collector|ownership flow status ingestion)\b/;
