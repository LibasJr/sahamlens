...
| Status di SahamLens | **Aktif** (agregat pasar resmi IDX; tanpa ticker/buy-sell) | **Aktif sebagai evidence layer, eksperimental** |
...
## 5. Status Broker Summary

Broker Summary **aktif** melalui pipeline resmi IDX `idx-flow-sync`. Endpoint
`TradingSummary/GetBrokerSummary` menghasilkan agregat EOD seluruh pasar per kode
broker dan menyimpannya ke `broker_market_daily`. Pipeline ini tidak memakai Index
Alpha.

Skema `broker_summary_daily` per-emiten dan data historisnya tetap dipertahankan,
tetapi bukan jalur aktif karena endpoint resmi IDX tidak menyediakan ticker atau
pemisahan buy/sell. Data agregat pasar tidak boleh dipaksakan menjadi net buy per
emiten.

Ownership Flow **bukan penggantinya**. Keduanya menjawab pertanyaan berbeda.
