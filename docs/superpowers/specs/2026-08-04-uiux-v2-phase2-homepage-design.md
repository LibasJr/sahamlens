...
| 5 | `:101,104,145-154,336-341,357` `loadingPicks/picksNeedPro/picksLoginRequired` + fetch `/api/recommendations` (`:143-154`) | **Dihapus seluruhnya** — diganti reuse `loadingRadar`/`radarError` (`:88-89`, sudah ada dari LensRadar) untuk Hero juga, karena sama-sama dari `/api/ai-pick`. |
| 6 | Baru, dekat state `radarItems` (`:85-87`) | Tambah state Pro-gate untuk `/api/ai-pick`: `const [picksNeedPro, setPicksNeedPro] = useState(false);` `const [picksLoginRequired, setPicksLoginRequired] = useState(false);` (nama dipertahankan sama seperti sebelumnya supaya minim perubahan di JSX yang mereferensikan keduanya untuk AI briefing gating `:300-303`). |
