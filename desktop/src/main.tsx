...
import { API_BASE_URL, checkHealth, type HealthState } from './api';
...
function App() {
  ...
  return <div className="terminal">
    ...
    <main className="main"><header className="header"><div><p className="eyebrow">MONDAY, 30 AUGUST 2026</p><h1>{active}</h1></div><div className="actions"><button className="search">⌕&nbsp; Search stock <kbd>⌘ K</kbd></button><button className="icon">◔</button><button className="icon" onClick={() => { setRefreshing(true); checkHealth(API_BASE_URL).then(setHealth).finally(() => setRefreshing(false)); }} aria-label="Refresh koneksi">{refreshing ? '…' : '↻'}</button><div className={`connection ${health}`}><i />{connectionLabel}</div><ViewToggle detail={detail} onChange={setDetail} /></div></header>
