...
function App() {
  ...
  return <div className="terminal">
    ...
    <main className="main"><header className="header"><div><p className="eyebrow">{new Intl.DateTimeFormat('id-ID', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }).format(new Date()).toUpperCase()}</p><h1>{active}</h1></div><div className="actions"><button className="search" onClick={() => setSearchOpen(!searchOpen)}>⌕&nbsp; Search stock <kbd>⌘ K</kbd></button>{searchOpen && <div className="search-box"><input autoFocus className="search-input" value={query} onChange={(e) => setQuery(e.target.value.toUpperCase())} onKeyDown={(e) => { if (e.key === 'Enter') selectSearchResult(); }} placeholder="Ticker, mis. BBCA" aria-label="Cari saham" />{query && <div className="search-results">{stocks.filter((s) => `${s.code} ${s.name}`.includes(query)).map((s) => <button key={s.code} onClick={() => { setQuery(s.code); setSearchOpen(false); }}><b>{s.code}</b><small>{s.name}</small></button>)}{!stocks.some((s) => `${s.code} ${s.name}`.includes(query)) && <span>Tidak ditemukan</span>}</div>}</div>}<button className="icon" onClick={refreshMarket} aria-label="Refresh data market">{refreshing ? '…' : '↻'}</button><button className="icon" onClick={() => setSettingsOpen(!settingsOpen)} aria-label="Buka pengaturan" aria-expanded={settingsOpen}>⚙</button>{settingsOpen && <div className="detail-hint" role="dialog">Pengaturan Desktop: API {API_BASE_URL}</div>}<div className={`connection ${health}`}><i />{connectionLabel}</div>{pulse?.timestamp && <span className="updated-at">Diperbarui {new Date(pulse.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>}<ViewToggle detail={detail} onChange={setDetail} /></div></header>
      ...
      <div className="columns"><section className="panel chart-panel"><div className="panel-title"><div><p className="eyebrow">{benchmark?.symbol ?? 'Benchmark'} · {chartRange}</p><h3>{benchmark ? formatPrice(benchmarkPrice) : '—'} <span className={benchmarkChange >= 0 ? 'gain' : 'loss'}>{benchmark ? changeLabel : 'Data belum tersedia'}</span></h3></div><select className="ghost" value={chartRange} onChange={(e) => setChartRange(e.target.value)} aria-label="Pilih rentang chart"><option>1D</option><option>1W</option><option>1M</option></select></div><div className="chart" role="img" aria-label={`Chart ${benchmark?.symbol ?? 'benchmark'} rentang ${chartRange}`}><div className="grid-lines" />{benchmark?.sparkline?.length ? <svg viewBox="0 0 700 230" preserveAspectRatio="none" aria-hidden="true"><polyline points={benchmark.sparkline.map((value, index, values) => `${(index / Math.max(values.length - 1, 1)) * 700},${220 - ((value - Math.min(...values)) / Math.max(Math.max(...values) - Math.min(...values), 1)) * 190}`).join(' ')} fill="none" stroke="#39d0b1" strokeWidth="3" vectorEffect="non-scaling-stroke" /></svg> : <div className="detail-hint">Seri historis belum tersedia untuk rentang ini.</div>}</div><div className="range"><span>{chartRange === '1M' ? '01 Aug' : chartRange === '1W' ? 'Sen' : '09:00'}</span><span>{chartRange === '1M' ? '08 Aug' : chartRange === '1W' ? 'Rab' : '11:00'}</span><span>{chartRange === '1M' ? '15 Aug' : chartRange === '1W' ? 'Jum' : '13:00'}</span><span>{chartRange === '1M' ? '31 Aug' : chartRange === '1W' ? 'Min' : '15:00'}</span></div></section><section className="panel"><div className="panel-title"><div><p className="eyebrow">WATCHLIST</p><h3>My stocks</h3></div><button className="plus">＋</button></div><div className="stock-list">{stocks.map(s => <div className="stock" key={s.code}><div className="stock-logo">{s.code[0]}</div><div className="stock-name"><b>{s.code}</b><small>{s.name}</small></div><div className="stock-price"><b>Rp {s.price}</b><small className={s.positive ? 'gain' : 'loss'}>{s.change}</small></div></div>)}</div><button className="view-all" onClick={() => { setWatchlistExpanded(!watchlistExpanded); setActive('Watchlist'); }} aria-expanded={watchlistExpanded}>View all watchlist <span>→</span></button>{watchlistExpanded && <div className="detail-hint">Menampilkan {stocks.length} saham dari data market terbaru.</div>}</section></div>
