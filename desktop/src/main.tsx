...
function App() {
  ...
  return <div className="terminal">
    ...
    <main className="main"><header className="header"><div><p className="eyebrow">MONDAY, 30 AUGUST 2026</p><h1>{active}</h1></div><div className="actions"><button className="search" onClick={() => setSearchOpen(!searchOpen)}>⌕&nbsp; {query || 'Search stock'} <kbd>⌘ K</kbd></button>{searchOpen && <div className="search-box"><input autoFocus className="search-input" value={query} onChange={(e) => setQuery(e.target.value.toUpperCase())} placeholder="Ticker, mis. BBCA" aria-label="Cari saham" />{query && <button className="search-clear" onClick={() => setQuery('')} aria-label="Hapus pencarian">×</button>}{query && <div className="search-results">{stocks.filter((s) => `${s.code} ${s.name}`.includes(query)).map((s) => <button key={s.code} onClick={() => { setQuery(s.code); setSearchOpen(false); }}><b>{s.code}</b><small>{s.name}</small></button>)}{!stocks.some((s) => `${s.code} ${s.name}`.includes(query)) && <span>Tidak ditemukan</span>}</div>}</div>}<button className="icon" onClick={refreshMarket} aria-label="Refresh data market">{refreshing ? '…' : '↻'}</button><button className="icon">⚙</button><div className={`connection ${health}`}><i />{connectionLabel}</div><ViewToggle detail={detail} onChange={setDetail} /></div></header>
      ...
      <div className="columns"><section className="panel chart-panel"><div className="panel-title"><div><p className="eyebrow">IHSG · 1D</p><h3>{chartRange === '1D' ? '7.812,34' : chartRange === '1W' ? '7.764,12' : '7.521,88'} <span className="gain">+48,20 (+0,62%)</span></h3></div><label className="range-select"><span className="sr-only">Rentang chart</span><select className="ghost" value={chartRange} onChange={(e) => setChartRange(e.target.value)} aria-label="Pilih rentang chart"><option value="1D">1D</option><option value="1W">1W</option><option value="1M">1M</option></select><span className="range-note">{chartRange === '1D' ? 'Hari ini' : chartRange === '1W' ? '7 hari' : '1 bulan'}</span></label></div><div className="chart"><div className="grid-lines" /><svg viewBox="0 0 700 230" preserveAspectRatio="none"><defs><linearGradient id="fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#39d0b1" stopOpacity=".26"/><stop offset="1" stopColor="#39d0b1" stopOpacity="0"/></linearGradient></defs><path d="M0 190 C50 170 65 180 105 155 S160 160 195 130 S240 145 280 110 S340 125 380 90 S420 120 460 76 S510 100 545 65 S600 90 640 42 S675 62 700 25 V230 H0Z" fill="url(#fill)"/><path d="M0 190 C50 170 65 180 105 155 S160 160 195 130 S240 145 280 110 S340 125 380 90 S420 120 460 76 S510 100 545 65 S600 90 640 42 S675 62 700 25" fill="none" stroke="#39d0b1" strokeWidth="3"/></svg></div><div className="range"><span>09:00</span><span>11:00</span><span>13:00</span><span>15:00</span></div></section><section className="panel"><div className="panel-title"><div><p className="eyebrow">WATCHLIST</p><h3>My stocks</h3></div><button className="plus">＋</button></div><div className="stock-list">{stocks.map(s => <div className="stock" key={s.code}><div className="stock-logo">{s.code[0]}</div><div className="stock-name"><b>{s.code}</b><small>{s.name}</small></div><div className="stock-price"><b>Rp {s.price}</b><small className={s.positive ? 'gain' : 'loss'}>{s.change}</small></div></div>)}</div><button className="view-all">View all watchlist <span>→</span></button></section></div>
