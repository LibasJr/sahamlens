...
function App() {
  ...
  useEffect(() => {
    ...
    return () => { mounted = false; window.clearInterval(timer); window.removeEventListener('keydown', onKeyDown); };
  }, []);
  ...
  return <div className="terminal">
    ...
    <main className="main"><header className="header"><div><p className="eyebrow">MONDAY, 30 AUGUST 2026</p><h1>{active}</h1></div><div className="actions"><button className="search">⌕&nbsp; Search stock <kbd>⌘ K</kbd></button><button className="icon" onClick={refreshMarket} aria-label="Refresh data market">{refreshing ? '…' : '↻'}</button><button className="icon">⚙</button><div className={`connection ${health}`}><i />{connectionLabel}</div><ViewToggle detail={detail} onChange={setDetail} /></div></header>
