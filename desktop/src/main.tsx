...
function App() {
  ...
  const syncWatchlist = () => { const requestId = authSequence.current; void getDesktopWatchlist().then((items) => { if (requestId !== authSequence.current) return; setSavedSymbols(items.map((item) => item.symbol)); setWatchlist((current) => { const existing = new Set(current.map((item) => item.symbol)); return [...current, ...items.filter((item) => !existing.has(item.symbol)).map((item) => ({ symbol: item.symbol, name: item.symbol, price: null, change: null }))]; }); setWatchlistSyncError(''); }).catch(() => { if (requestId === authSequence.current) { setSavedSymbols([]); setWatchlistSyncError('Masuk untuk sinkronisasi Watchlist.'); } }); };
  const refreshAccount = () => { const requestId = authSequence.current; void getAccount().then((payload) => { if (requestId === authSequence.current) setAccount((payload as { user?: DesktopAccount }).user ?? null); }).catch(() => { if (requestId === authSequence.current) setAccount(null); }); };
  useEffect(() => { syncWatchlist(); refreshAccount(); const listener = () => { authSequence.current += 1; syncWatchlist(); refreshAccount(); }; window.addEventListener('desktop-auth-changed', listener); return () => { authSequence.current += 1; window.removeEventListener('desktop-auth-changed', listener); }; }, []);
