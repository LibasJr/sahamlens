...
import { API_BASE_URL, checkHealth, getMarketSummary, type HealthState, type MarketSummary } from './api';
import './styles.css';
...
function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([checkHealth(API_BASE_URL), getMarketSummary(API_BASE_URL).catch(() => null)]).then(([state, summary]) => {
      if (!cancelled) { setHealth(state); setMarket(summary); }
    });
    return () => { cancelled = true; };
  }, []);
  const marketChange = market?.marketRegime.changePct ?? null;
  const marketValue = marketChange === null ? '—' : `${marketChange >= 0 ? '+' : ''}${marketChange.toFixed(2)}%`;
  const healthLabel = health === 'connected' ? 'Data tersambung' : health === 'offline' ? 'Koneksi terputus' : 'Memeriksa koneksi';
  return <div className="terminal">
    ...
    <main className="main"><header className="header"><div><p className="eyebrow">MONDAY, 30 AUGUST 2026</p><h1>{active}</h1></div><div className="actions"><button className="search">⌕&nbsp; Search stock <kbd>⌘ K</kbd></button><button className="icon">◔</button><button className="icon">⚙</button><div className={`connection ${health}`}><i />{healthLabel}</div></div></header>
      ...
      <section className="metrics"><Metric label="IHSG" value="—" change={marketValue} positive={marketChange !== null && marketChange >= 0} /><Metric label="Foreign Flow" value="+Rp 284,6 M" change="Net Buy" positive /><Metric label="Market Breadth" value="187 / 142" change="Advancers / Decliners" /></section>
