...
async function getJson<T>(url: string): Promise<T> { const response = await fetch(url, { signal: AbortSignal.timeout(10000) }); if (!response.ok) { throw new Error(`HTTP ${response.status} dari ${new URL(url).pathname}`); } return response.json() as Promise<T>; }
export async function checkHealth(baseUrl = ''): Promise<HealthState> { try { return (await fetch(`${baseUrl}/api/health`, { signal: AbortSignal.timeout(8000) })).ok ? 'connected' : 'offline'; } catch { return 'offline'; } }
