...
export function CalendarWorkspace() {
  ...
  const load = () => { const requestId = ++requestSequence.current; setLoading(true); setError(''); void requestFeature('/api/calendar').then((value) => { if (requestId === requestSequence.current) setData(value as CalendarPayload); }).catch((reason) => { if (requestId === requestSequence.current) setError(reason instanceof Error ? reason.message : 'Kalender belum dapat dimuat.'); }).finally(() => { if (requestId === requestSequence.current) setLoading(false); }); };
