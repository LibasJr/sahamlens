...
type Props = { market: { summary: MarketSummary; pulse: MarketPulse } | null; error: string; onSelect: (symbol: string) => void; hideBreadth?: boolean };

export function MarketOverview({ market, error, onSelect, hideBreadth = false }: Props) {
  ...
  return <section className="market-overview">
    ...
    {!hideBreadth && breadth && <div className="breadth-strip"><div><span>MARKET BREADTH</span><strong>{breadth.raw?.advanceShare == null ? '—' : `${breadth.raw.advanceShare.toFixed(1)}%`}</strong><small>saham sampel menguat</small></div><div><span>ADVANCERS</span><strong className="positive">{breadth.raw?.advancing ?? '—'}</strong></div><div><span>DECLINERS</span><strong className="negative">{breadth.raw?.declining ?? '—'}</strong></div><div><span>SKOR BREADTH</span><strong>{breadth.score ?? '—'}</strong></div></div>}
