...
export function TitleBar({ apiStatus = 'checking' }: { apiStatus?: 'checking' | 'online' | 'offline' }) {
  ...
  return <header className="titlebar" data-tauri-drag-region>
    <div className="titlebar-brand"><span className="brand-glyph"><Activity size={15} /></span><span>SahamLens</span><span className="titlebar-divider" /><span className="titlebar-context">RESEARCH DESKTOP</span><span>API {apiStatus === 'checking' ? 'CHECKING' : apiStatus === 'online' ? '200 OK' : 'OFFLINE'}</span></div>
